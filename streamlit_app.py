...
for i, dia in enumerate(itinerario):
    ...
    if dia["día"] == 1:
        ...
        seleccion = st.radio("Selecciona el vehículo y aeropuerto", list(opciones_transporte.keys()), key=f"vehiculo_{i}")
        precio_vehiculo = opciones_transporte[seleccion] if usar_vehiculo else 0
    else:
        opciones_vehiculo = ["Hiace", "Alphard"]
        seleccion = st.radio("Selecciona el vehículo", opciones_vehiculo, key=f"vehiculo_{i}")
        precio_vehiculo = dia[seleccion] if usar_vehiculo else 0

    st.markdown("**Selecciona el hotel:**")
    hoteles_disponibles = df_hoteles["Nombre"].tolist()
    precios_hoteles = df_hoteles["Precio_con_Impuestos"].tolist()
    lista_hoteles = [f"{nombre} – ¥{precio:,}" for nombre, precio in zip(hoteles_disponibles, precios_hoteles)]
    ...
    vehiculo = st.selectbox(f"Vehículo", opciones_vehiculo, key=f"vehiculo_{i}") if opciones_vehiculo else ""
