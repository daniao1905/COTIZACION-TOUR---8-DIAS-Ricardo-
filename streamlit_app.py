...
for i, dia in enumerate(itinerario):
    ...
    hoteles_disponibles = df_hoteles[df_hoteles["Ciudad"].str.strip().str.lower() == ciudad.strip().lower()]["NombreHotel"].tolist()
    precios_hoteles = (df_hoteles[df_hoteles["Ciudad"].str.strip().str.lower() == ciudad.strip().lower()]["Precio"] + df_hoteles[df_hoteles["Ciudad"].str.strip().str.lower() == ciudad.strip().lower()]["Impuesto"]).tolist()
    lista_hoteles = [
    f"{row['NombreHotel']} – ¥{row['Precio'] + row['Impuesto']:,}"
    for _, row in df_hoteles[df_hoteles["Ciudad"].str.strip().str.lower() == ciudad.strip().lower()].iterrows()
]
    hotel_seleccionado = st.selectbox("NombreHotel", lista_hoteles, key=f"hotel_select_{i}")
    opciones_vehiculo = []
    if dia["día"] == 1:
    ...
    hotel_dropdown = [f"{row['NombreHotel']} – ¥{int(row['Total por noche con impuesto (JPY)']):,}" for _, row in hoteles_disponibles.iterrows()]
