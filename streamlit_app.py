...
for i, dia in enumerate(itinerario):
    ...
    hoteles_disponibles = df_hoteles[
        (df_hoteles["Ciudad"] == ciudad) &
        (df_hoteles["Tipo de día"].str.lower() == tipo_dia.lower())
    ]
    hotel_dropdown = [f"{row['Hotel']} – ¥{row['Total por noche con impuesto (JPY)']:,}" for _, row in hoteles_disponibles.iterrows()]
    ...
    if hotel_seleccionado:
        ...
        row = hoteles_disponibles[hoteles_disponibles["Hotel"] == hotel_nombre]
        if not row.empty:
            precio_hotel = row["Total por noche con impuesto (JPY)"].values[0]
    if hotel_seleccionado:
        hotel_nombre = hotel_seleccionado.split(" – ¥")[0]
        precio_hotel = hoteles_disponibles[hoteles_disponibles["Hotel"] == hotel_nombre]["Total por noche con impuesto (JPY)"].values[0]
